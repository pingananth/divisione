...
const ROLES = ['All', 'President', 'VPE', 'VPM', 'VPPR', 'Sec', 'Treas', 'SAA', 'Leaders', 'Trainers', 'Role Players', 'ClubExcomOfficer'];

export default function BreakTheIceClient({ initialAttendees }: { initialAttendees: Attendee[] }) {
  ...
  }, [initialAttendees, searchTerm, selectedDivision, selectedArea, selectedRole]);

  const totalConsented = initialAttendees.filter(a => a.consent).length;
                  ...
                  <div className="flex-shrink-0">
                    {attendee.image ? (
                      <Image
                        ...
                        width={80}
                        height={80}
                        className="rounded-lg object-cover bg-zinc-700 border border-zinc-600 h-20 w-20"
                        loading="lazy"
                      />
                    ) : (
                      <div className="rounded-lg bg-zinc-700 border border-zinc-600 h-20 w-20 flex items-center justify-center text-zinc-500 font-bold text-xl uppercase">
